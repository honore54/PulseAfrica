...
export default async function CategoryPage({ params, searchParams }) {
  ...
  let articles = []
  try {
    articles = await getArticles({ category, lang, limit: 24 })
  } catch {}
  ...
  return (
    <>
      ...
      {/* Category hero */}
      <div style={{
        background: `linear-gradient(160deg,var(--pure) 0%,${cat.light} 100%)`,
        ...
        <div style={{ maxWidth:1380, margin:'0 auto' }}>
          ...
          <h1 style={{ fontFamily:"'Cormorant',serif", fontWeight:300, fontSize:'clamp(48px,6vw,80px)', color:'var(--ink)', lineHeight:.95, letterSpacing:-1.5, marginBottom:14 }}>
            {cat.label}
          </h1>
          ...
          <>
            {featured && (
              <div style={{ marginBottom:14 }}>
                <ArticleCard article={featured} big lang={lang} />
              </div>
            )}
